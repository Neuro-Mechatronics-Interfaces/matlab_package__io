...
add_to_output(filename);
add_to_output(path);

add_to_output(notes);
add_to_output(frequency_parameters);
if (data_file_main_version_number > 1)
    add_to_output(reference_channel);
end

if (num_amplifier_channels > 0)
    add_to_output(amplifier_channels);
    if (data_present)
        add_to_output(amplifier_data);
        add_to_output(t_amplifier);
    end
    add_to_output(spike_triggers);
end
if (num_aux_input_channels > 0)
    add_to_output(aux_input_channels);
    if (data_present)
        add_to_output(aux_input_data);
        add_to_output(t_aux_input);
    end
end
if (num_supply_voltage_channels > 0)
    add_to_output(supply_voltage_channels);
    if (data_present)
        add_to_output(supply_voltage_data);
        add_to_output(t_supply_voltage);
    end
end
if (num_board_adc_channels > 0)
    add_to_output(board_adc_channels);
    if (data_present)
        add_to_output(board_adc_data);
        add_to_output(t_board_adc);
    end
end
if (num_board_dig_in_channels > 0)
    add_to_output(board_dig_in_channels);
    if (data_present)
        add_to_output(board_dig_in_data);
        add_to_output(t_dig);
    end
end
if (num_board_dig_out_channels > 0)
    add_to_output(board_dig_out_channels);
    if (data_present)
        add_to_output(board_dig_out_data);
        add_to_output(t_dig);
    end
end
if (num_temp_sensor_channels > 0)
    if (data_present)
        add_to_output(temp_sensor_data);
...
% return
    ...
    function add_to_output(variable)

        % add_to_output(variable)
        %
    ...
    function s = plural(n)
        %PLURAL Utility function to optionally pluralize words based on value of n.
        %
        % s = plural(n)
    ...
    function out = notch_filter(in, fSample, fNotch, Bandwidth)
        %NOTCH_FILTER  Implements a 50- or 60-Hz notch filter.
        %
