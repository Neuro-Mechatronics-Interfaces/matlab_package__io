function data = read_Intan_RHS2000_file(fname, options)
...
if (num_amplifier_channels > 0)
    ...
    if (data_present)
        ...
        if (dc_amp_data_saved ~= 0)
            add_to_output(dc_amplifier_data);
        end
        add_to_output(stim_data);
        add_to_output(amp_settle_data);
        add_to_output(charge_recovery_data);
        add_to_output(compliance_limit_data);
        add_to_output(t);
    end
...
if (num_board_dac_channels > 0)
    add_to_output(board_dac_channels);
    if (data_present)
        add_to_output(board_dac_data);
    end
end
if (num_board_dig_in_channels > 0)
